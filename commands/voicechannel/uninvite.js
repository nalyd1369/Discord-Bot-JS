...
module.exports = {
    name: 'uninvite',
    category: 'Testing',
    description: 'Uninvites a user from your locked call',
    expectedArgs: '<user>',
    minArgs: 1,
    slash: true,
    options: [{
        name: 'user',
        description: 'Who to ban from call',
        type: 'MENTIONABLE',
        required: true,
    }],
    ...
    callback: ({ interaction, member }) => {
        ...
        calls.findOne({ callID: call.id }, async function (err, found) {
            ...
            if (found.ownerID != member.id && !member.permissions.has('MANAGE_CHANNELS')) { errHandler('noPerm', interaction) }
            
            call.permissionOverwrites.edit(mentioned, { VIEW_CHANNEL: false });
            if (mentioned.user) {
                mentioned.voice.disconnect().catch(err => console.log(err))
            }
            const Claim = new MessageEmbed()
                .setAuthor(`| You have banned ${mentioned.displayName} from the call`, `${member.displayAvatarURL()}`)
                .setColor(`#d0c7be`)
            interaction.reply({ embeds: [Claim], ephemeral: true })
        }).clone()//fix
        trackCommands('uninvite')
    },
}